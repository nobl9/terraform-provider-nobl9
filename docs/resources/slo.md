...
### Zscaler application metrics

Zscaler SLOs require an existing Zscaler data source and service.
For an Agent data source, change the indicator kind to `Agent`.

Supported metrics are `score` (0–100), `pft` and `dns` (milliseconds), and `availability` (percent).
Application and location IDs are positive integers. Omit `location_id` to aggregate across all locations.
These are raw metrics, not good/total counts. Threshold objectives measure the percentage of collected samples that meet
the configured condition, not a percentage of individual users. If ZDX returns only page fetch time for a different requested
metric, the query returns no data.
...
- `queries` (Block List) ABC pattern-based query configuration. Specify individual queries in letter-labeled rows (A-F), where subsequent queries can reference previous queries (e.g., Query #C can reference Query #A and Query #B). The final query defines the SLI for this objective. Up to six queries per SLI, including the resulting formula query (see [below for nested schema](#nestedblock--objective--count_metrics--bad--sumologic--queries))
- `query` (String, Deprecated) Query for the metrics. Deprecated: use the 'queries' block instead.
...
- `query` (String) The Sumo Logic query. Subsequent queries can reference previous queries (e.g., Query #C can reference Query #A and Query #B). The final query defines the SLI for this objective.
...
<a id="nestedblock--objective--count_metrics--bad--zscaler"></a>
### Nested Schema for `objective.count_metrics.bad.zscaler`
...
<a id="nestedblock--objective--count_metrics--good--zscaler"></a>
### Nested Schema for `objective.count_metrics.good.zscaler`
...
<a id="nestedblock--objective--count_metrics--good_total--zscaler"></a>
### Nested Schema for `objective.count_metrics.good_total.zscaler`
...
<a id="nestedblock--objective--count_metrics--total--zscaler"></a>
### Nested Schema for `objective.count_metrics.total.zscaler`
